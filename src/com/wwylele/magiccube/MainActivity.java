    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(new GLSurfaceView(this) {
            ...
            private Cube cube;
            {
                ...
                cube=((MainActivity) getContext()).cube;
                renderer = new MainRenderer(getContext().getResources(), cube);
            ...
            private void pointToModelSpace(float x,float y,float[] from, float[] direction){
               /*float[] projSpace={
                        (x/getWidth())*2-1,
                        (1.0f-y/getHeight())*2-1,
                        0,1};
                float[] viewSpaceFrom={0,0,0,1},viewSpaceTo=new float[4];
                float[] worldSpaceFrom=new float[4],worldSpaceTo=new float[4];
                float[] modelSpaceFrom=new float[4],modelSpaceTo=new float[4];
               
                Matrix.multiplyMV(viewSpaceTo, 0, renderer.invProj, 0, projSpace, 0);
                Matrix.multiplyMV(worldSpaceFrom, 0, renderer.invView, 0, viewSpaceFrom, 0);
                Matrix.multiplyMV(worldSpaceTo, 0, renderer.invView, 0, viewSpaceTo, 0);
                Matrix.multiplyMV(modelSpaceFrom, 0, cube.invModel, 0, worldSpaceFrom, 0);
                Matrix.multiplyMV(modelSpaceTo, 0, cube.invModel, 0, worldSpaceTo, 0);
                for(int i=0;i<3;++i){
                    from[i]=modelSpaceFrom[i]/modelSpaceFrom[3];
                    modelSpaceTo[i]/=modelSpaceTo[3];
                    direction[i]=modelSpaceTo[i]-from[i];
                }*/
                ...
                float[] modelSpaceFrom=new float[4],modelSpaceTo=new float[4];
                GLU.gluUnProject(x, getHeight()-y, 1.0f, mtxMV, 0, renderer.mtxProj, 0, view, 0, modelSpaceFrom, 0);
                GLU.gluUnProject(x, getHeight()-y, 0.0f, mtxMV, 0, renderer.mtxProj, 0, view, 0, modelSpaceTo, 0);
                for(int i=0;i<3;++i){
                    from[i]=modelSpaceFrom[i]/modelSpaceFrom[3];
            ...
            private static final int MOTION_NONE=0,MOTION_ROTATING=1,MOTION_TURN=2,MOTION_TURNING=3;
